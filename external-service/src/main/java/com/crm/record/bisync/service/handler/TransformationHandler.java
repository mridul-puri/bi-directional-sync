...
public class TransformationHandler extends AbstractHandler {
    ...
    public void handle(ProcessingContext context) {
        if(context.getOperationType().name().equals(OperationType.SYNC)) {
            //Transformation for Sync Message

            String contact = transformationService.transform(Constants.SYNC_FROM + context.getSyncMessage().getSource(), context.getSyncMessage().getContact());
            context.getSyncMessage().setContact(contact);
        } else if(context.getProvider().endsWith(Constants.REVERSE)) {
